...
def _smbv1_scan_os(remote_name, remote_host, sess_port=445, timeout=10,):
    try:
        ...
        return {
            'smb_version': 1,
            'is_login_required': smb1_client.is_login_required(),
            'is_signing_required': smb1_client.is_signing_required(),
            'remote_host': smb1_client.get_remote_host(),
            'remote_name': smb1_client.get_remote_name(),
            'server_dns_domain_name': smb1_client.get_server_dns_domain_name(),
            'server_dns_host_name': smb1_client.get_server_dns_host_name(),
            'server_domain': smb1_client.get_server_domain(),
            'server_lanman': smb1_client.get_server_lanman(),
            'server_name': smb1_client.get_server_name(),
            'server_os': smb1_client.get_server_os(),
            'server_os_build': smb1_client.get_server_os_build(),
            'server_os_major': smb1_client.get_server_os_major(),
            'server_os_minor': smb1_client.get_server_os_minor(),
            'server_time': smb1_client.get_server_time(),
        }
...
def _smbv3_scan_os(remote_name, remote_host, sess_port=445, timeout=10,):
    try:
        ...
        return {
            'smb_version': 2,
            'is_login_required': smb3_client.is_login_required(),
            'is_signing_required': smb3_client.is_signing_required(),
            'remote_host': smb3_client.get_remote_host(),
            'remote_name': smb3_client.get_remote_name(),
            'server_dns_domain_name': smb3_client.get_server_dns_domain_name(),
            'server_dns_host_name': smb3_client.get_server_dns_host_name(),
            'server_domain': smb3_client.get_server_domain(),
            'server_name': smb3_client.get_server_name(),
            'server_os': smb3_client.get_server_os(),
            'server_os_build': smb3_client.get_server_os_build(),
            'server_os_major': smb3_client.get_server_os_major(),
            'server_os_minor': smb3_client.get_server_os_minor(),
        }
    except Exception as err:
...
@click.command()
@click.option('-h', '--host', 'remote_host', help='remote host')
@click.option('-n', '--name', 'remote_name', default='', show_default=True, help='remote name, if not given, will be set equal to `host`')
@click.option('-p', '--port', 'sess_port', default=445, show_default=True, help='remote port')
@click.option('-t', '--timeout', default=10, show_default=True, help='connection timeout')
@click.option('-v', '--version', 'smb_version', default='auto',  show_default=True, type=click.Choice(['1', '2', '3', 'auto']), help='smb version, if set to `auto`, will try smb v1 and v2(v3) in order until success or final failure')
def smb_scan_os(remote_host, remote_name=None, sess_port=445, timeout=10, smb_version=None):
    """
